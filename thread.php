...
WHERE
  ...
  $page = isset($_GET['id']) ? $repository->parseThread($_GET['id']) : null;
  if($page != null) {
    $_SESSION['reply-thread'] = ['id' => $page->id, 'open' => $page->open];
  }
...
<html lang="es">
  ...
  <body>
      ...
      <div class="threads">Hilos</div>
        <div class="threads-list">
          <?php
            ...
            <div class="<?php echo $page == $thread ? $id . " selected" : $id; ?>" onclick="
    ...
    <div class="thread-content" id="feed">
        <?php
            
            $repository = new Repository();
            $thread = isset($_GET['id']) ? $repository->parseThread($_GET['id']) : null;
            ...
            <div class="post-text">
                <div class="account">
                  <div class="profile">
                    <div class="name">
                            <p><?php echo $thread->user->name ?></p>
                        </div>
                        
                        <?php 
                        if ($thread->user->verified == 1) {
                        ?>
                        <div class="verified">
                    ...
                    <div class="date">
                        <p><?php echo epochTime($thread->date, true) ?></p>
                    </div>
                ...
                <?php 
                  ...
                  $images_encoded = json_encode($thread->images);


                  if(count($thread->images) != 0) {
                ?>

                <div class="<?php echo count($thread->images) > 1 ? "images multiple" : "images" ?>">

                <?php

                  if(count($thread->images) > 1) {

                ?>
                    <div class="primary">
                        <img src="<?php echo $thread->images[0] ?>"
                        data-images='<?php echo htmlspecialchars($images_encoded, ENT_QUOTES, 'UTF-8'); ?>'
                        onclick="openModal('<?php echo $thread->images[0] ?>', JSON.parse(this.dataset.images), true)">
                    </div>
                    <div class="secondary">
                        <?php if(count($thread->images) > 2) {  ?>
                          <img src="<?php echo $thread->images[1] ?>"
                        data-images='<?php echo htmlspecialchars($images_encoded, ENT_QUOTES, 'UTF-8'); ?>'
                        onclick="openModal('<?php echo $thread->images[1] ?>', JSON.parse(this.dataset.images), true)"
                        class="first">
                        <img src="<?php echo $thread->images[2] ?>"
                        data-images='<?php echo htmlspecialchars($images_encoded, ENT_QUOTES, 'UTF-8'); ?>'
                        onclick="openModal('<?php echo $thread->images[2] ?>', JSON.parse(this.dataset.images), true)"
                        class="second">
                        <?php } else { ?>
                          <img src="<?php echo $thread->images[1] ?>"
                        data-images='<?php echo htmlspecialchars($images_encoded, ENT_QUOTES, 'UTF-8'); ?>'
                        onclick="openModal('<?php echo $thread->images[1] ?>', JSON.parse(this.dataset.images), true)">
                        <?php }
                        if(count($thread->images) >= 4) {   
                            ?>
                          <div class="overlay" src="<?php echo $thread->images[2] ?>" data-images='<?php echo htmlspecialchars($images_encoded, ENT_QUOTES, 'UTF-8'); ?>'
                          onclick="openModal('<?php echo $thread->images[2] ?>', JSON.parse(this.dataset.images), true)"></div>
                            <p src="<?php echo $thread->images[2] ?>" data-images='<?php echo htmlspecialchars($images_encoded, ENT_QUOTES, 'UTF-8'); ?>'
                            onclick="openModal('<?php echo $thread->images[2] ?>', JSON.parse(this.dataset.images), true)"><?php echo "+" . (count($thread->images) - 2); ?></p>
                            <?php
                ...
                ?>
                  ...
                  $survey = $repository->parseSurvey($thread);
                  ...
                  <div class="survey">
                      ...
                      <div class="options">
                      <?php 
                        $votes = array_sum($survey->votes);
                        foreach($survey->votes as $key => $value) {
                        $selected = array_key_exists($thread->id, $profile->surveys) && ($profile->surveys[$thread->id] == $key || ($survey->multi_select == 1 && in_array($key, $profile->surveys[$thread->id])));
                      ?>
                        
                        <div class="<?php echo $selected ? 'option selected' : 'option'; ?>"  onclick="vote(this, '<?php echo $thread->id; ?>', '<?php echo $key; ?>', <?php echo $survey->multi_select; ?>)">
                        
                            <div class="box">
                                <div class="progress" style="max-width: <?php echo $votes == 0 ? 0 : round(($value * 100) / $votes, 2); ?>%;"></div>
                                <p><?php echo $key ?></p><span><?php echo $votes == 0 ? 0 : round(($value * 100) / $votes, 2); ?></span>
                        ...
                        ?>
                          ...
                          <?php
                            if($votes > 1 || $votes == 0) {
                              ?>
                              <p class="votes"><?php echo $votes . ' votos' ?></p>
                              <?php
                            } else if($votes == 1) {
                              ?>
                              <p class="votes"><?php echo $votes . ' voto' ?></p>
                              <?php
                ...
                <div class="reaction">
                    ...
                    <div class="like">
                        ...
                        if($profile->isLiked($thread->id) != 1) {
                      ?>
                        <div class="icon" onclick="like(this, 'thread', '<?php echo $thread->id; ?>')">
                  ...
                  <?php
                      ...
                      ?>
                        <div class="icon clicked" onclick="like(this, 'thread', '<?php echo $thread->id; ?>')">
                        <svg aria-label="Ya no me gusta" role="img" viewBox="0 0 48 48"><title>Ya no me gusta</title><path d="M34.6 3.1c-4.5 0-7.9 1.8-10.6 5.6-2.7-3.7-6.1-5.5-10.6-5.5C6 3.1 0 9.6 0 17.6c0 7.3 5.4 12 10.6 16.5.6.5 1.3 1.1 1.9 1.7l2.3 2c4.4 3.9 6.6 5.9 7.6 6.5.5.3 1.1.5 1.6.5s1.1-.2 1.6-.5c1-.6 2.8-2.2 7.8-6.8l2-1.8c.7-.6 1.3-1.2 2-1.7C42.7 29.6 48 25 48 17.6c0-8-6-14.5-13.4-14.5z"></path></svg>
                        </div>
            ...
            <?php 
              if($profile->id == $thread->user->id || $profile->verified == 1) {
            ?>
            ...
            <div class="options">
              <div class="button" onclick="
                  ...
                  if(selectedElement == '<?php echo $thread->id ?>' || selectedElement == '') {
                      optionsList.classList.toggle('show');
                  }
                  selectedElement = '<?php echo $thread->id; ?>'
      ...
      <form action="reply-thread.php" method="post" enctype="multipart/form-data" style="display: none;">
      <div class="post-account">
        <img alt="<?php echo $profile->id . '\'s icon'; ?>" src="<?php echo isset($profile->icon) ? $profile->icon : 'profile-none.jpg'; ?>" />
      </div>
<div class="post-text">
...
<input type="submit" value="Subir">
        ...
        <?php
            ...
            $posts = $repository->getPosts($thread);

            if(count($posts) == 0) {
              ?>
              </div>
                <div class="none">
                  <p>Este hilo todavia no tiene respuestas.</p>
                </div>
              <?php
